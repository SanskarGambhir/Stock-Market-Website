...
function Pay() {
  ...
  // Check wallet connection and auto-connect if available
  useEffect(() => {
  ...
  // JSX for the Pay component
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 flex flex-col lg:flex-row p-4 mt-15">
      <div className="flex flex-1 flex-col lg:flex-row gap-10 justify-between w-full">
        {/* Left Column */}
        <div className="flex-1 bg-white/10 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-white/20">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-300 text-center">Secure payment powered by Razorpay</p>
          </div>

          <div className="mb-6">
            <label className="block text-gray-200 text-sm font-medium mb-2">Choose Amount</label>
            <div className="grid grid-cols-4 gap-2 mb-4">
              {quickAmounts.map((quickAmount) => (
                <button
                  ...
                  className={`py-2 rounded-lg transition-all ${amount === quickAmount.toString() ? "bg-indigo-600 text-white" : "bg-white/20 text-white hover:bg-white/30"}`}
                >
                  ${quickAmount}
                </button>
              ))}
          ...
          <button
            className={`w-full py-4 px-6 rounded-xl text-lg font-medium transition-all ${isLoading ? "bg-gray-500 cursor-not-allowed" : "bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg hover:shadow-xl"}`}
